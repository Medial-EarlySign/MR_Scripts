...
./b2 cxxflags=-fPIC link=static variant=release -j$(nproc) pch=off --stagedir="${WORK_BUILD_FOLDER}/Boost" --with-program_options --with-system --with-regex --with-filesystem
...
./b2 cxxflags=-fPIC pch=off link=shared variant=release -j$(nproc) --stagedir="${WORK_BUILD_FOLDER}/Boost" --with-program_options --with-system --with-regex --with-filesystem