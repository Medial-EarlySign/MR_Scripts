...
1. Compile the `AlgoMarker_Server` wrapper (located in the `Tools` directory), and copy the resulting executable into `data/app`. Please use version that compiled in manylinux2014
...
4. Compile the AlgoMarker library (in manylinux2014) and place the shared library in: `data/app/$ALGOMARKER_NAME/${AM_NAME}_${AM_VERSION}/lib`
...
## Result
Currently it crashes in Alpine 