...
then
	if command -v sudo &> /dev/null; then
		sudo apt update
		sudo apt install binutils perl htop ncdu net-tools vim bzip2 python3-pip python3-dev libpython3-dev libgomp1 gcc zip p7zip-full openssh-server gawk dos2unix cgdb sed file systemtap-sdt-dev make less libffi-dev swig cmake lsof libssl-dev zlib1g-dev locate postgresql libpq-dev ccrypt libsqlite3-dev curl freetds-bin freetds-dev iputils-ping libbz2-dev lzma nano screen tdsodbc unixodbc-dev unzip -y
	else
		apt update
		apt install binutils perl htop ncdu net-tools vim bzip2 python3-pip python3-dev libpython3-dev libgomp1 gcc zip p7zip-full openssh-server gawk dos2unix cgdb sed file systemtap-sdt-dev make less libffi-dev swig cmake lsof libssl-dev zlib1g-dev locate libpython3 postgresql libpq-dev ccrypt libsqlite3-dev curl freetds-bin freetds-dev iputils-ping libbz2-dev lzma nano screen tdsodbc unixodbc-dev unzip -y
	fi
...
cd ${BASE_PATH}/libs; tar -xvf ${FP_PATH}/libs.tar.bz2; tar -xvf ${FP_PATH}/PY.tar.bz2
cd ${BASE_PATH}/bins; tar -xvf ${FP_PATH}/bin_apps.x86-64.tar.bz2
cd ${BASE_PATH}/scripts; tar -xvf ${FP_PATH}/scripts.tar.bz2; tar -xvf ${FP_PATH}/ETL.tar.bz2

mv ${BASE_PATH}/scripts/Scripts/* ${BASE_PATH}/scripts/
rmdir ${BASE_PATH}/scripts/Scripts
...
ONLY_UPDATE=$(egrep "AUTOTEST_LIB|ETL_LIB_PATH" ~/.bashrc | wc -l | awk '{print $1 - 1}')
if [ $ONLY_UPDATE -lt 1 ]; then
...
#dpkg -L libpython3.10 | egrep "\.so$" #To locate libpython3.so and create symbolic link
#PY_LIB_PATH=$(dpkg -L libpython3.10 | egrep "\.so$")
#ln -s ${PY_LIB_PATH} $BASE_PATH/libs/libpython3.so

ln -s ${BASE_PATH} ${BASE_PATH}/

mkdir -p ${BASE_PATH}/workspace
mkdir -p ${BASE_PATH}/workspace/notebooks
ln -s ${BASE_PATH} ${BASE_PATH}/workspace/notebooks
echo "jupyter-lab --ip 0.0.0.0 --allow-root --port 7002 --no-browser --notebook-dir ${BASE_PATH}/workspace/notebooks --ServerApp.terminado_settings=\"shell_command=['/bin/bash']\" --IdentityProvider.token='' --ServerApp.password=''" > $BASE_PATH/scripts/Bash-Scripts/start_jupyter
