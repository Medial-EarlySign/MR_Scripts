...
#c.JupyterHub.data_files_path = '/opt/medial/python36/usr/share/jupyterhub'
...
c.Spawner.environment = {
    'IPYTHONDIR': lambda self,sp =__import__('subprocess'): sp.run(
    [ 'bash','-c','mkdir -p /var/opt/medial/jupyter/'+self.user.name+' && echo -n /var/opt/medial/jupyter/'+self.user.name ]
...
c.Spawner.notebook_dir = '/nas1/Work/Shared/notebooks'
...
#c.Authenticator.admin_users = set('shlomi')
