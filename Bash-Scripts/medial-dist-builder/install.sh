...
PIP_INSTALL_LIST="${CFDIR}/pip_install_list.txt"
PIP_REPODIR="${DLDIR}/${PYNAM}-repo"
mkdir -p $PIP_REPODIR
if [ ! -f "${PIP_INSTALL_F}" ] ; then
echo "${INFOP}Downloading package from ${PIP_INSTALL_LIST}"
rm -f ${LGDIR}/pip_${PYNAM}_err.txt ${LGDIR}/pip_${PYNAM}_out.txt
for i in `cat ${PIP_INSTALL_LIST}`; do bash -c ". ${ENABLE_FILE} && pip download -d $PIP_REPODIR $i" ; done 2>> ${LGDIR}/pip_${PYNAM}_err.txt 1>>${LGDIR}/pip_${PYNAM}_out.txt
touch ${PIP_INSTALL_F}
...
PIP_SITEINSTALL_LIST="${CFDIR}/pip_install_list.txt"
if [ ! -f "${PIP_SITEINSTALL_F}" ] ; then
echo "${INFOP}Installing package from ${PIP_SITEINSTALL_LIST}"
rm -f ${LGDIR}/pip_siteinstall_${PYNAM}_err.txt ${LGDIR}/pip_siteinstall_${PYNAM}_out.txt
for i in `cat ${PIP_INSTALL_LIST}`; do bash -c ". ${ENABLE_FILE} && pip install --find-links=file://${PIP_REPODIR} $i" ; done 2>> ${LGDIR}/pip_siteinstall_${PYNAM}_err.txt 1>>${LGDIR}/pip_siteinstall_${PYNAM}_out.txt
...
mkdir -p $PIP_REPODIR
if [ ! -f "${PIP_INSTALL_F}" ] ; then
echo "${INFOP}Installing package from ${PIP_INSTALL_LIST}"
rm -f ${LGDIR}/pip_${PYNAM}_err.txt ${LGDIR}/pip_${PYNAM}_out.txt
for i in `cat ${PIP_INSTALL_LIST}`; do bash -c ". ${ENABLE_FILE} && pip download -d $PIP_REPODIR $i" ; done 2>> ${LGDIR}/pip_${PYNAM}_err.txt 1>> ${LGDIR}/pip_${PYNAM}_out.txt
touch ${PIP_INSTALL_F}
#cat ${LGDIR}/pip_${PYNAM}_out.txt  | egrep -v '^(  Runnung|  Downloading|Collecting|Successfully|Requirement|  Stored|Building|Installing|  Running)'
