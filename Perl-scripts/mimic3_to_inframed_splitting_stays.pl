...
# Global Parameters
my @dataSFiles = qw/Demographics_S ICU_Stays_S LabEvents_S ChartEvents_S IOEvents_S ICD9 PROCEDURES/ ;
my @dataNFiles = qw/Demographics_N ICU_Stays_N  LabEvents_N  ChartEvents_N IOEvents_N CoMorbidities MicroBiologyEvents_N ExtraEvents PoeOrder_N/ ;
...
sub doDemographics {
	...
	foreach my $line (@$lines) {
		...
		if (! exists $idData->{stays}->{$hadmId}) {
			...
			for my $colName (qw/MARITAL_STATUS ETHNICITY INSURANCE RELIGION ADMISSION_TYPE ADMISSION_LOCATION/) {
				...
				if ($desc ne "") {
					...
					foreach my $stay (@{$idData->{stays}->{$hadmId}}) {
						...
						push @{$idData->{data}->{$stay}->{Demographics_S}},[$signal,$stayInTime,$desc]  ;
			...
			foreach my $stay (@{$idData->{stays}->{$hadmId}}) {
				push @{$idData->{data}->{$stay}->{Demographics_N}},[$signal,$dischTime,$hospAlive] ;
...
sub doTransfers {
	...
	my $TimeToUniteSubstays = 60 ; # Unite stays that are up to 60 minutes apart
	my $maxStaysPerId = 5 ;
	...
	foreach my $stayID (keys %stayRecs) {
		...
		# Idenitfy substays
		my @subStays ;
		push @subStays,[$inds[0]] ;
		for my $i (1..$#inds) {
			if ($recs[$inds[$i]]->{inTime} - $recs[$inds[$i-1]]->{outTime} > $TimeToUniteSubstays) {
				$logFH->print("TRSF : Splitting stay $inId/$stayID\n") ;
				push @subStays,[$inds[$i]] ;
			} else {
				push @{$subStays[-1]},$inds[$i];
			}
		}
		
		# Split
		my (@splitStays,@newStayIds);
		foreach my $subStay (@subStays) {
			my $inTime = $recs[$subStay->[0]]->{inTime} ;
			my $outTime = $recs[$subStay->[-1]]->{outTime} ;			
			push @splitStays,[$inTime,$outTime] ;
		}
		handleSubStays($idData,$allData,$stayID,\@splitStays,\@newStayIds) ;

		# Add Extra Signals
		# OrigStayID
		foreach my $id (@newStayIds) {	
			push @{$idData->{data}->{$id}->{ICU_Stays_N}},["OrigStayID",$idData->{data}->{$id}->{InTime},$stayID] ;
			$allData->{SignalValuesPerStay}->{$stayID}->{OrigStayID}->{$id} = 1 ;
		}
		
		# Source/Destination and Care_Units
		my ($source,$discharge) ;
		for my $i (0..$#splitStays) {
			my $subStay = $subStays[$i] ;
			my $newId = $newStayIds[$i] ;
			my ($inTime,$outTime) = @{$splitStays[$i]} ;
			
			# Source
			if ($recs[$subStay->[0]]->{event} eq "admit") {
				$source = "hospitalAdmission" ;
			} else {
				$logFH->print("TRSF : ICU admission from ICU for $inId/$stayID\n") if ($recs[$subStay->[0]]->{prevCareUnit} ne "") ;
				$source = "ward" ;
			}
			addToDict($allData,$source,"") if (! exists $allData->{dict}->{$source})  ;
			addToSignalValues($allData,"source",$source) ;
			push @{$idData->{data}->{$newId}->{ICU_Stays_S}},["source",$inTime,$source] ;
			$allData->{SignalValuesPerStay}->{$newId}->{"source"}->{$source} = 1 ;
			
			# Destination
			if ($recs[$subStay->[-1]+1]->{event} eq "discharge") {
			...
			push @{$idData->{data}->{$newId}->{ICU_Stays_S}},["dischargeDestination",$outTime,$discharge] ;
			$allData->{SignalValuesPerStay}->{$newId}->{"dischargeDestination"}->{$discharge} = 1 ;

			# Care Units
			my @careUnits = map {[$recs[$_]->{currCareUnit},$recs[$_]->{inTime},$recs[$_]->{outTime}]} grep {$recs[$_]->{currCareUnit} ne ""} @$subStay ;
			if (@careUnits) {
				push @{$idData->{data}->{$newId}->{ICU_Stays_S}},["First_Care_Unit",$careUnits[0]->[1],$careUnits[0]->[0]] ;
				...
				$allData->{SignalValuesPerStay}->{$newId}->{First_Care_Unit}->{$careUnits[0]->[0]} = 1 ;

				foreach my $unitRec (@careUnits) {
					push @{$idData->{data}->{$newId}->{ICU_Stays_S}},["Care_Units",$unitRec->[1],$careUnits[0]->[2],$unitRec->[0]] ;
					addToDict($allData,$unitRec->[0],"") if (! exists $allData->{dict}->{$careUnits[0]->[0]})  ;
					addToSignalValues($allData,"Care_Units",$unitRec->[0]) ;							
					$allData->{SignalValuesPerStay}->{$newId}->{Care_Units}->{$unitRec->[0]} = 1 ;
				}
			}
		}		
	}
...
# Handle sub-ICU stays
sub handleSubStays {
	my ($idData,$allData,$stayId,$splitStays,$newStayIds) = @_ ;

	my @splitStays = @$splitStays ;
			
	if (@splitStays > 1) {
		@$newStayIds = map {$stayId + $_ * $staySplittingShift} (0..$#splitStays) ;
		foreach my $type (@dataFiles) {				
			if ($type eq "LabEvents_N" or $type eq "IOEvents_N" or $type eq "MicroBiologyEvents_N" or $type eq "PoeOrder_N" or $type eq "Demographics_S" or $type eq "Demographics_N" or $type eq "LabEvents_S" or
				$type eq "IOEvents_S" or $type eq "ICD9" or $type eq "PROCEDURES" or $type eq "ExtraEvents" or $type eq "CoMorbidities") { # Multiply for all sub-stays
				for $idx (1..$#splitStays) {
					my $splitId = $stayId + $idx * $staySplittingShift ;
					map {push @{$idData->{data}->{$splitId}->{$type}},$_} @{$idData->{data}->{$stayId}->{$type}};
				}						
			} elsif ($type eq "ChartEvents_N" or $type eq "ChartEvents_S") { # Split to sub-stays by time
				my @recs = @{$idData->{data}->{$stayId}->{$type}} ;
				@{$idData->{data}->{$stayId}->{$type}} = () ;
				foreach my $rec (@recs) {
					my $idx = getSplitIdx($rec,\@splitStays) ;
					if ($idx == -1) {
						my @rec = @$rec ;
						$logFH->print("SPLT : Problems assigning sub-stay to '@rec' in stay $stayId\n") ;
					} else {
						push @{$idData->{data}->{$stayId + $idx * $staySplittingShift}->{$type}},$rec ;
					}
				}
			} elsif ($type eq "ICU_Stays_S" or $type eq "ICU_Stays_N") { # Change
				my @recs = @{$idData->{data}->{$stayId}->{$type}} ;
				@{$idData->{data}->{$stayId}->{$type}} = () ;
				
				foreach my $rec (@recs) {
					if ($rec->[0] eq "ID" or $rec->[0] eq "Age" or $rec->[0] eq "HospitalAdmin" or $rec->[0] eq "Gender") {
						for $idx (0..$#splitStays) {
							my $splitId = $stayId + $idx * $staySplittingShift ;
							push @{$idData->{data}->{$splitId}->{$type}},$rec ;
						}
					} elsif ($rec->[0] eq "InTime") {
						for my $idx (0..$#splitStays) {
							my @info = @$rec ;
							$info[1] = $splitStays[$idx]->[0] ;
							my $splitId = $stayId + $idx * $staySplittingShift ;
							push @{$idData->{data}->{$splitId}->{$type}},\@info ;
							$idData->{data}->{$splitId}->{InTime} = $info[1] ;
						}
					} elsif ($rec->[0] eq "OutTime") {
						for my $idx (0..$#splitStays) {
							my @info = @$rec ;
							$info[1] = $splitStays[$idx]->[1] ;
							my $splitId = $stayId + $idx * $staySplittingShift ;
							push @{$idData->{data}->{$splitId}->{$type}},\@info ;
							$idData->{data}->{$splitId}->{OutTime} = $info[1] ;
						}
					} elsif ($rec->[0] eq "Care_Units" or $rec->[0] eq "Destination" or $rec->[0] eq "First_Care_Unit") {
						my $idx = getSplitIdx($rec,\@splitStays) ;
						if ($idx == -1) {
							my @rec = @$rec ;
							$logFH->print("SPLT : Problems assigning sub-stay to '@rec' in stay $stayId\n") ;
						...
						die "Cannot handle rec->[0] in splitting\n" ;	
					}
				}
			} else {
				die "Cannot handle data type $type in splitting to substays" ;
			}
		}
	} else {
		@$newStayIds = ($stayId) ;
...
sub getSplitIdx {
	...
	my $nFields = scalar @$rec ;
	
	if ($nFields == 3) { # TimeVal
		my $time = $rec->[1] ;
		foreach my $idx (0..(scalar(@$times)-1)) {
			return $idx if ($time >= $times->[$idx]->[0] and $time <= $times->[$idx]->[1]) ;
		}
		return -1 ;
	} elsif ($nFields == 4) {
		my ($startTime,$endTime) = ($rec->[1],$rec->[2]) ;
		foreach my $idx (0..(scalar(@$times)-1)) {
			return $idx if ($startTime >= $times->[$idx]->[0] and $endTime <= $times->[$idx]->[1]) ;
		}
		return -1 ;	
	} else {
		die ;
