...
die "Usage : $0 InDirFile OutDir FileOfFilesToUnite" if (@ARGV!=3) ;
my ($inDirFile,$outDir,$filesToUnite) = @ARGV ;
...
# Unite data files :
uniteDataFiles(\@inDirs,$outDir,$filesToUnite) ;
...
sub uniteDataFiles {
	my ($dirs,$outDir,$filesToUnite) = @_ ;
	
	my $n = scalar @$dirs ;
	
	# Read list of files to unite
	my @fileNames ;
	open (IN,$filesToUnite) or die "Cannot open $filesToUnite for reading" ;
	while (<IN>) {
		chomp ;
		push @fileNames,$_ ;
	}
	...
	foreach my $fileName (@fileNames) {
		...
		if ($pid == -1) {
			...
			exec "sort -u -S50% $outDir/$fileName.tmp | sort -n -k1,1 -S50%  > $outDir/$fileName" or die;
		}
