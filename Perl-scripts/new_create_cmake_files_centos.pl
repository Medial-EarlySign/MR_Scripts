...
### main ###
my $p = {
	cmake_top_level_templ => "$ENV{MR_ROOT}/Projects/Resources/CMakeUtils/CmakeTopLevelTemplate.txt",
	...
	for my $uniqProjName (@{$solCmake->{$solName}{projList}}) {
		...
		if ($projCmake->{$uniqProjName}{isLib} == 1) {
			...
			if ($projCmake->{$uniqProjName}{depXGBoostPO}) {
				...
				#push @linkLibList, "/nas1/Work/SharedLibs/linux/lib64/\${CMAKE_BUILD_TYPE}/librabit.a";
			}
			if ($projCmake->{$uniqProjName}{depVWPO} && $p->{new_compiler} > 0) {
				#push @linkLibList, "/usr/local/lib/libvw.so";
				#push @linkLibList, "/usr/local/lib/libvw_c_wrapper.so";
			}
